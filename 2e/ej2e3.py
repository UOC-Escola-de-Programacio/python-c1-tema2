...
def create_app():
    ...
    def post_xml():
        ...
        if request.content_type != "application/xml":
            return jsonify({"error": "El Content-Type no es xml"}), 400
    ...
    def post_image():
        ...
        if request.content_type not in ['image/png', 'image/jpeg']:
            return jsonify({"error": "El Content-Type debe ser image/png o image/jpeg"}), 400
    ...
    def post_binary():
        ...
        if request.content_type != 'application/octet-stream':
            return jsonify({"error": "El Content-Type debe ser application/octet-stream"}), 400
